...
// Get the entertainment place ID from the URL
$entertainment_id = isset($_GET['id']) ? intval($_GET['id']) : 0;

if ($entertainment_id > 0) {
    // Fetch entertainment details
    $stmt = $conn->prepare("SELECT * FROM places WHERE place_id = ?");
    $stmt->bind_param("i", $entertainment_id);
    $stmt->execute();
    $result = $stmt->get_result();
    $entertainment = $result->fetch_assoc();

    if (!$entertainment) {
        echo "Entertainment place not found.";
        exit;
    }

    // Fetch all images related to the place_id
    $image_stmt = $conn->prepare("SELECT image_path FROM place_images WHERE place_id = ?");
    $image_stmt->bind_param("i", $entertainment_id);
    $image_stmt->execute();
    $image_result = $image_stmt->get_result();

    // Fetch reviews
    $review_stmt = $conn->prepare("SELECT name, review, review_date FROM reviews WHERE place_id = ? ORDER BY review_date DESC");
    $review_stmt->bind_param("i", $entertainment_id);
    $review_stmt->execute();
    $reviews_result = $review_stmt->get_result();
} else {
    echo "Invalid entertainment ID.";
    exit;
}
?>
...
<head>
    ...
    <title><?= htmlspecialchars($entertainment['name']) ?> - Details</title>
...
<div class="container mt-4">
    <h1><strong><?= htmlspecialchars($entertainment['name']) ?></strong></h1>
    <p> • <?= htmlspecialchars($entertainment['category']) ?>  📍 <?= htmlspecialchars($entertainment['location_address']) ?></p>
    <p>🎟 Entry Fee: <?= htmlspecialchars($entertainment['entry_fee']) ?></p>
    <p>🕓 <?= htmlspecialchars($entertainment['opening_hours']) ?></p>
    <p>📞 <?= htmlspecialchars($entertainment['phone']) ?> &nbsp;&nbsp; ⭐ <?= number_format($entertainment['ratings'], 1) ?></p>
    
    <p>
    <?php if (!empty($entertainment['maps'])): ?>
...
<div class="modal fade" id="mapModal" tabindex="-1" aria-labelledby="mapModalLabel" aria-hidden="true">
    ...
    <div class="row">
        <?php while ($image = $image_result->fetch_assoc()): ?>
            <div class="col-md-4">
                <img src="<?= htmlspecialchars('../' . $image['image_path']) ?>" alt="<?= htmlspecialchars($entertainment['name']) ?>" class="img-fluid rounded" style="max-height: 300px; object-fit: cover; width: 100%;">
            </div>
        <?php endwhile; ?>
    </div>
</div>
...
<div class="container mt-4">
    <form id="checklistForm">
        <input type="hidden" name="place_id" value="<?= $entertainment_id ?>">
        ...
        <div id="memoriesBox" class="memories-box mt-3">
            <textarea name="memories" class="form-control" placeholder="Share your memories here..."></textarea>
        </div>
...
$(document).on('click', '#openMapModal', function() {
    var mapUrl = "<?= htmlspecialchars($entertainment['maps']) ?>";
    if (mapUrl.trim() !== "") {
