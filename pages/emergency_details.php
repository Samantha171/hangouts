...
// Get the emergency place ID from the URL
...
<head>
    ...
    <title><?= htmlspecialchars($emergency['name']) ?> - Details</title>
...
<div class="container mt-4">
    <h1><strong><?= htmlspecialchars($emergency['name']) ?></strong></h1>
    <p> • <?= htmlspecialchars($emergency['category']) ?> 📍 <?= htmlspecialchars($emergency['location_address']) ?></p>
    
    <?php if (!empty($emergency['entry_fee'])): ?>
        <p>🎟 Entry Fee: <?= htmlspecialchars($emergency['entry_fee']) ?></p>
    <?php endif; ?>
    
    <p>🕓 <?= htmlspecialchars($emergency['opening_hours']) ?></p>
    
    <?php if (!empty($emergency['emergency_number'])): ?>
        <p>🚨 Emergency Number: <?= htmlspecialchars($emergency['emergency_number']) ?></p>
    <?php endif; ?>
    
    <p>⭐ <?= number_format($emergency['ratings'], 1) ?></p>
    
    <p>
    <?php if (!empty($emergency['maps'])): ?>
...
<div class="modal fade" id="mapModal" tabindex="-1" aria-labelledby="mapModalLabel" aria-hidden="true">
    ...
    <div class="row">
        <?php while ($image = $image_result->fetch_assoc()): ?>
            <div class="col-md-4">
                <img src="<?= htmlspecialchars('../' . $image['image_path']) ?>" alt="<?= htmlspecialchars($emergency['name']) ?>" class="img-fluid rounded" style="max-height: 300px; object-fit: cover; width: 100%;">
            </div>
        <?php endwhile; ?>
    </div>
</div>
...
<div class="container mt-4">
    <form id="checklistForm">
        <input type="hidden" name="place_id" value="<?= $emergency_id ?>">
        ...
        <div class="mt-3">
            ...
            <input type="date" id="visited_on" name="visited_on" class="form-control" required>
        </div>
...
<div class="container mt-5">
    ...
    var mapUrl = "<?= htmlspecialchars($emergency['maps']) ?>";
    if (mapUrl.trim() !== "") {
